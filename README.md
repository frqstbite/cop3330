# COP3330

This repository contains the code, scriptwriting, and video projects behind my
COP3330 assignments. The course syllabus is available online [here][syllabus].

## Project Structure

Each assignment has its own project directory within the root directory of this
repository. The project directories follow a common structure, described below.

### Code

The starter file for each assignment is available in an accompanying
`driver.cpp`, if provided. Each assignment contains a `solution` directory that
contains my attempted solution.

### Devlog

Each assignment has a `devlog.md` file describing the authentic, real-time
experience of writing and debugging the program. This devlog will primarily
server to inform the scriptwriting process.

### Script

After the program has been sufficiently tested and the devlog has been
finalized, the script is written to communicate the key points in the video.
Code snippets are included to supplement the visuals added in post.

### Video

## License

This repository is not released under a license. You do not have permission to
copy, use, or redistribute this repository or its contents.

[syllabus]: https://gaosh.github.io/courses/cop_3330_spring26
[motioncanvas]: https://motioncanvas.io